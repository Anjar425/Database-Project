@foreach ($transmigrasi as $data)
    <div id="editModal{{ $data->IDMigrasi }}"
        ...
        <div class="bg-gray-800 rounded-lg w-1/2">
            <form method="POST" action="{{ url('/' . $data->IDMigrasi . '/update-transmigrasi') }}"
                class=" w-5/6 mx-auto my-5">
                @csrf
                <h2 class=" text-center font-semibold text-lg text-white">Edit Transmigrasi</h2><br>
                <div class="mb-5">
                ...
                <div class="mb-5 basis-1/3">
                    <label for="LokasiTransmigrasi"
                        class="block mb-2 text-sm font-medium  text-white">LokasiTransmigrasi</label>
                    <input name="LokasiTransmigrasi" type="text" id="LokasiTransmigrasi"
                        class=" border  text-sm rounded-lg  block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500"
                        value="{{ $data->LokasiTransmigrasi }}">
                </div>

                <div class="mb-5 basis-1/3">
                    <label for="AlamatAsal" class="block mb-2 text-sm font-medium text-white">AlamatAsal</label>
                    <input name="AlamatAsal" type="text" id="AlamatAsal"
                        class=" border  text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500"
