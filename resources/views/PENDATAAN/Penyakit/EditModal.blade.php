@foreach ($penyakit as $data)
    <div id="editModal{{ $data->IDPenyakit }}"
        class="hidden fixed inset-0 bg-gray-400 bg-opacity-60 justify-center items-center">
        <div class="bg-gray-800 rounded-lg w-1/2">
            <form method="POST" action="{{ url('/' . $data->IDPenyakit . '/update-penyakit') }}"
                class=" w-5/6 mx-auto my-5">
                @csrf
                <h2 class=" text-center font-semibold text-lg text-white">Edit Penyakit</h2><br>
                <div class="mb-5">
                    <label for="IDPenyakit" class="block mb-2 text-sm font-medium  text-white">ID Penyakit</label>
                    <input name="IDPenyakit" type="text" id="IDPenyakit"
                        class="border   text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500"
                        value="{{ $data->IDPenyakit }}" required>
                </div>
                
                    <div class="mb-5 basis-1/3">
                        <label for="NamaPenyakit"
                            class="block mb-2 text-sm font-medium  text-white">Nama Penyakit</label>
                        <input name="NamaPenyakit" type="text" id="NamaPenyakit"
                            class=" border  text-sm rounded-lg  block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500"
                ...
                <div class="flex flex-row gap-3">
                    ...
                    <button type="button" onclick="closeEditModal('{{ $data->IDPenyakit }}')"
